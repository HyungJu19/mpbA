...
CREATE TABLE person (
                        ...
                        id varchar(255) UNIQUE not null ,            -- 고유 식별자 (기본 키)
...
CREATE TABLE role (
                      no INT AUTO_INCREMENT PRIMARY KEY, -- 권한 ID (PK)
...
CREATE TABLE pers_role (
                      ...
                      FOREIGN KEY (person_id) REFERENCES person(no) ON DELETE CASCADE,
                      FOREIGN KEY (role_id) REFERENCES role(no) ON DELETE CASCADE
);
